...
from data_transfer.shared_drive_data_transfer.ctera_shared_drive_data_import_job import move_data_from_remote_server, prepare_config, ConnectionInfo, check_file_transfer_status
...
def dag_context_fixture() -> dict:
    mock_dag = MagicMock()
    mock_dag.dag_id = "ctera_shared_drive_data_import_job"

    mock_dag_run = MagicMock()
    mock_dag_run.conf = {
        "dag_id": "ctera_shared_drive_data_import_job",
        "remote_filepath": "testing",
        "lookup_key": ["dfspcbalmuat", "nonprod"],
        "marker_file_bucket": "terminus-erm-np",
        "marker_file_prefix": "data_transfer/ctera_import/artifacts/sales_data_2019.csv.d59fd82e-4fbb-4248-a040-78f32c856854",
        "destination_bucket": "terminus-erm-np",
        "source_dag_id": "ctera_shared_drive_data_import",
        "source_dag_run_id": "manual__2025-05-26T12:31:33+00:00",
        "destination_path": "inbound/sales_data_2019.csv",
        "destination_project_id": "pcb-nonprod-erm",
        "source_type": "file",
        "team_name": "erm",
        "trigger_type": "manual"
    }
    ...
    return {
        ...
        'run_id': 'manual__2025-05-26T11:08:34+00:00'
    }
...
def audit_record_fixture() -> AuditRecord:
    expected_audit_record_values = """
        'pcb-nonprod-erm',
        'ctera_shared_drive_data_import',
        'manual__2025-05-26T12:31:33+00:00',
        'file',
        'gs://terminus-erm-np/inbound/sales_data_2019.csv',
        'sales_data_2019.csv',
        'pcb-nonprod-erm',
        'testing',
        'ctera_shared_drive_data_import_job',
        'manual__2025-05-26T11:08:34+00:00',
        'Shared_Drive_Data_Import',
        'manual',
        'pcb-nonprod-erm',
        CURRENT_TIMESTAMP()
    """
...
class TestCteraSharedDriveExportJob:
    ...
    @patch('data_transfer.shared_drive_data_transfer.ctera_shared_drive_data_import_job.read_yamlfile_env')
    @patch('data_transfer.shared_drive_data_transfer.ctera_shared_drive_data_import_job.SMBUtil')
    @patch('data_transfer.shared_drive_data_transfer.ctera_shared_drive_data_import_job.resolve_smb_server_config')
    def move_data_from_remote_server(
        ...
        mock_ctera_util.copy_ctera_to_gcs.return_value = 'SUCCESS'

        # Call the move_data_from_remote_server function
        move_data_from_remote_server(
            remote_filepath='testing',
            source_file_name='sales_data_2019.csv',
            destination_bucket='terminus-erm-np',
            destination_path='inbound/sales_data_2019.csv',
        ...
        mock_ctera_util.copy_gcs_to_ctera.assert_called_once_with(
            'DFSPCBALMUAT/UAT/poc_data_movement/testing', 'sales_data_2019.csv', 'terminus-erm-np', None, 'inbound/sales_data_2019.csv'
        )
    ...
    @patch('data_transfer.shared_drive_data_transfer.ctera_shared_drive_data_import_job.AuditUtil')
    @patch('data_transfer.shared_drive_data_transfer.ctera_shared_drive_data_import_job.AuditUtil.record_request_received')
    @patch('data_transfer.shared_drive_data_transfer.ctera_shared_drive_data_import_job.create_data_transfer_audit_table_if_not_exists')
    def test_prepare_config(
        ...
        marker_file_bucket_call = call.xcom_push(key='marker_file_bucket', value='terminus-erm-np')
        marker_file_prefix_call = call.xcom_push(key='marker_file_prefix', value='data_transfer/ctera_import/artifacts/sales_data_2019.csv.d59fd82e-4fbb-4248-a040-78f32c856854')
        connect_info_call = call.xcom_push(key='connection_info', value={"source": "dfspcbalmuat", "environment": "nonprod"})
        audit_record_call = call.xcom_push(key='audit_record', value=asdict(audit_record_call_arg))
        source_bucket_call = call.xcom_push(key='destination_bucket', value='terminus-erm-np')
        source_path_call = call.xcom_push(key='destination_path', value='inbound/sales_data_2019.csv')
        remote_filepath_call = call.xcom_push(key='remote_filepath', value='testing')
        source_file_name_call = call.xcom_push(key='source_file_name', value='sales_data_2019.csv')

        mock_ti.assert_has_calls([marker_file_bucket_call, marker_file_prefix_call, connect_info_call, audit_record_call,
                                  source_bucket_call, source_path_call, remote_filepath_call, source_file_name_call])
    ...
    def test_check_file_transfer_status(self):
        ...
        def get_context_with_state(state: str):
            ...
            dummy_ti.task_id = 'move_data_from_remote_server'
        ...
        with pytest.raises(AirflowException, match="Task move_data_from_remote_server failed. Hence, Failing the DAG."):
