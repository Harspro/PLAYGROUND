...
AS (
  WITH
    LATEST_ACCESS_MEDIUM AS (
      ...
      QUALIFY
        ROW_NUMBER()
          OVER (PARTITION BY account_customer_uid ORDER BY update_dt DESC)
        = 1
    ),
    ...
    CONTACT AS (
      SELECT
        ...
        MAX(IF(UPPER(type) = 'ADDRESS', contact_uid, NULL))
          AS address_contact_uid
  ...
  SELECT
    ...
    NULLIF(CAST(LATEST_ACCESS_MEDIUM.access_medium_uid AS STRING), '')
      AS accessMediumUid,
    IF(
  ...
  JOIN pcb-{env}-processing.domain_account_management.ACTIVE_CUSTOMER_BASE
    ON C2P_ELIGIBLE_CUSTOMERS.customer_uid = ACTIVE_CUSTOMER_BASE.customer_uid
  JOIN pcb-{env}-curated.domain_customer_management.CUSTOMER
  ...
  LEFT JOIN pcb-{env}-curated.domain_customer_management.ADDRESS_CONTACT
    ON CONTACT.address_contact_uid = ADDRESS_CONTACT.contact_uid
  LEFT JOIN LATEST_ACCESS_MEDIUM
    ON
      ACTIVE_CUSTOMER_BASE.account_customer_uid
      = LATEST_ACCESS_MEDIUM.account_customer_uid
  LEFT JOIN pcb-{env}-curated.domain_customer_management.CUSTOMER_PREFERENCE
    ON
      CUSTOMER.customer_uid = CUSTOMER_PREFERENCE.customer_uid
      AND UPPER(CUSTOMER_PREFERENCE.code) = 'LANGUAGE'
  LEFT JOIN pcb-{env}-curated.domain_customer_management.CUSTOMER_PREFERENCE_PARAM
    ON
      CUSTOMER_PREFERENCE.customer_preference_uid
      = CUSTOMER_PREFERENCE_PARAM.customer_preference_uid
  LEFT JOIN OPTED_OUT_CUSTOMERS
    ON CUSTOMER.customer_uid = OPTED_OUT_CUSTOMERS.customerId
  LEFT JOIN pcb-{env}-processing.domain_account_management.CLOSED_BLOCK_CODES
    ON
      CUSTOMER.customer_uid = CLOSED_BLOCK_CODES.customer_uid
      AND ACTIVE_CUSTOMER_BASE.account_no = CLOSED_BLOCK_CODES.cifp_account_id5
  WHERE
    CUSTOMER.customer_uid IN (
      14655066, 14656115, 14656117, 9043074, 2538188, 4671657, 4452232, 13815303, 14656114, 14656120, 14176495)
    AND OPTED_OUT_CUSTOMERS.customerId IS NULL
