WITH
  ...
  WHERE
    a.product_uid=7
    AND (b.customer_uid IN (13862612,
        14348914,
        13815303,
        12457570,
        13769133)
      OR (a.account_uid=11617666))),
  SAVINGS_ACCOUNT AS(
  SELECT
  ...
  WHERE
    CAST(a.cpa_transaction_type AS INTEGER) IN (200,
      ...
      606,
      612)
    AND DATE(a.fundsmove_processing_dt) BETWEEN DATE_SUB(CURRENT_DATE(),INTERVAL 15 DAY)
    AND CURRENT_DATE()
  GROUP BY
    account_no),
  DD_THRESHOLD AS(
