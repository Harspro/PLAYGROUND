-- {env}_reverse_etl.bi_query_telpay_ods definition
-- Drop table
DROP table if exists {env}_reverse_etl.bi_query_telpay CASCADE;

DROP table if exists {env}_reverse_etl.bi_query_telpay_ods CASCADE;

DROP table if exists {env}_reverse_etl.bi_query_telpay_ods_default CASCADE;

CREATE table if not exists {env}_reverse_etl.bi_query_telpay_ods (
	...
	constraint PK_TELPAY_ODS primary KEY(
		telpay_pymt_uid,
		update_year,
		update_month
	)
...
CREATE TABLE IF NOT EXISTS {env}_reverse_etl.bi_query_telpay_ods_default PARTITION OF {env}_reverse_etl.bi_query_telpay_ods DEFAULT;