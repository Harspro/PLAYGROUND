-- {env}_reverse_etl.bi_query_edi_ods definition
-- Drop table
DROP table if exists {env}_reverse_etl.bi_query_edi CASCADE;

DROP table if exists {env}_reverse_etl.bi_query_edi_ods CASCADE;

DROP table if exists {env}_reverse_etl.bi_query_edi_ods_default CASCADE;

CREATE table if not exists {env}_reverse_etl.bi_query_edi_ods (
    ...
    constraint PK_EDI_ODS primary KEY(
        edi_pymt_uid,
        update_year,
        update_month
    )
...
CREATE TABLE IF NOT EXISTS {env}_reverse_etl.bi_query_edi_ods_default PARTITION OF {env}_reverse_etl.bi_query_edi_ods DEFAULT;