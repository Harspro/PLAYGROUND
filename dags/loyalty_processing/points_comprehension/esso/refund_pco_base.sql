-- Aggregation logic for Daily/Monthly/Initial Load ESSO Refund PCO Base.
CREATE OR REPLACE TABLE
  `{staging_table_id}`
OPTIONS (
  expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 14 DAY)
) AS
WITH esso_bonus_and_pco_base AS (
  SELECT
  ...
  AND LOWER(cnsld_latest.wallettransaction_type) = 'return'
  AND cnsld_latest.account_campaign_id = '100000000'
  AND LOWER(cnsld_latest.event) = 'debit'
...
esso_bonus AS (
  ...
  AND LOWER(cnsld_latest.wallettransaction_location_storeparentid) in ('lpqx37','lpjr92','lphf21')
  AND LOWER(cnsld_latest.event) = 'unredeem'
  AND LEFT(ee_payment.tender_type, 6) in
  (
    '516179' --Velvet
    ,'518127' --Silver-Preload
  ...
  AND LOWER(ee_payment.payment_type) = 'creditcard'
  GROUP BY pcf_customer_id, parentwallettransactionid, {tx_year_month} card_bin
  HAVING points_earned > 0
)
SELECT
esso_bonus_and_pco_base.pcf_customer_id as pcf_customer_id,
{tx_year_month_expr_2}
esso_bonus_and_pco_base.card_bin as card_bin,
'ESSO' AS loyalty_partner,
'PB' AS earning_category,
'REFUND' AS transaction_type,
'IN_STORE_AND_ONLINE' AS purchase_method,
SAFE_CAST(
  (SUM(
    esso_bonus_and_pco_base.points_earned
    -
    (
      CASE
        WHEN esso_bonus.points_earned IS NULL THEN 0
        ELSE esso_bonus.points_earned
      END
    )
  ) * -1) AS INTEGER
) AS points_earned,
...
FROM esso_bonus_and_pco_base
LEFT JOIN esso_bonus ON esso_bonus_and_pco_base.parentwallettransactionid = esso_bonus.parentwallettransactionid
