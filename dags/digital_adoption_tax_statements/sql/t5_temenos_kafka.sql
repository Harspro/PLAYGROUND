EXPORT DATA
  OPTIONS (
    uri = 'gs://pcb-{env}-staging-extract/T5-Statement/statement_temenos_t5_tax-*.parquet',
    format = 'PARQUET',
    overwrite = true
  )
AS
...
SELECT
    ...
    FROM pcb-{env}-processing.domain_tax_slips.T5_ENRICHED_TAX_SLIP T5Slip
        ...
        LEFT JOIN pcb-{env}-landing.domain_tax_slips.T5_TAX_RAW_TRAILER TH
            ON 1 = 1
);