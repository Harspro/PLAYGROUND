...
(
    ...
    FROM `pcb-{env}-landing.domain_tax_slips.RL3_TAX_SLIP` s
    JOIN `pcb-{env}-landing.domain_tax_slips.RL3_TAX_HEADER` h
    ON s.SbmtRefId = h.SbmtRefId
    WHERE s.SbmtRefId = '{sbmt_ref_id}'
    AND s.BeneficiaireNo IN ({ids})
    ORDER BY NoReleve
)