import json
import logging
from abc import ABC
import pendulum
from airflow.exceptions import AirflowFailException
from airflow.operators.trigger_dagrun import TriggerDagRunOperator
from airflow.utils.trigger_rule import TriggerRule
from google.cloud import storage
import util.constants as consts
from airflow import settings
from airflow.models import Variable
from airflow.exceptions import AirflowException
from typing import Final, Union
from datetime import timedelta, datetime
from google.cloud import bigquery
from airflow.operators.python import PythonOperator, BranchPythonOperator
from airflow import DAG
from airflow.operators.empty import EmptyOperator as DummyOperator
from airflow.utils.task_group import TaskGroup
from util.deploy_utils import (
    read_pause_unpause_setting,
    pause_unpause_dag
)
from util.miscutils import (
    read_variable_or_file,
    read_yamlfile_env_suffix,
    read_env_filepattern,
    save_job_to_control_table, read_file_env)
from dag_factory.terminus_dag_factory import add_tags
import numpy as np

logger = logging.getLogger(__name__)

GCP_CONFIG = read_variable_or_file('gcp_config')
DEPLOY_ENV = GCP_CONFIG['deployment_environment_name']
LOCAL_TZ = pendulum.timezone('America/Toronto')
CURRENT_DATETIME = datetime.now(tz=LOCAL_TZ)
JOB_DATE_TIME = f"{CURRENT_DATETIME.strftime('%Y%m%d%H%M%S')}"
BATCH_NUM_TABLE: Final = 'batch_num_table'
SQL_FILE_PATH = f'{settings.DAGS_FOLDER}/pcmc_estatement_processing/sql/'
OUTPUT_FILE_CREATION: Final = 'output_file_creation'
FETCH_SEQUENCE_NUMBER_TABLE: Final = 'fetch_sequence_number_table'
FETCH_SEQUENCE_NUMBER_FIELD: Final = 'fetch_sequence_number_field'
SEQ_NUMBER_FIELD_PLACEHOLDER: Final = '{seq_number_field}'
SEQ_NUMBER_TABLE_PLACEHOLDER: Final = '{seq_number_table}'
FILE_CREATE_DT_PLACEHOLDER: Final = '{file_create_dt}'
OUTPUT_TABLE_PLACEHOLDER: Final = '{output_table_name}'
FILE_NAME: Final = '{file_name}'
INITIAL_SEQ_VALUE_PLACEHOLDER: Final = '{initial_seq_value}'
BATCH_NUM_PLACEHOLDER: Final = '{batch_num_val}'
CYCLE_NUM_PLACEHOLDER: Final = '{cycle}'


def get_output_folder_path(dag_config: dict):
    return f"gs://{dag_config[consts.PROCESSING_BUCKET]}/{dag_config.get('PROCESSING_OUTPUT_DIR')}"


def get_interim_folder_path(dag_config: dict):
    return f"gs://{dag_config[consts.PROCESSING_BUCKET]}/{dag_config.get('PROCESSING_INTERIM_DIR')}"


class OutputFileCreation(ABC):

    def __init__(self, config_filename: str, config_dir: str = None):
        self.gcp_config = read_variable_or_file(consts.GCP_CONFIG)
        self.deploy_env = self.gcp_config[consts.DEPLOYMENT_ENVIRONMENT_NAME]
        self.dataproc_config = read_variable_or_file(consts.DATAPROC_CONFIG)

        if config_dir is None:
            config_dir = f'{settings.DAGS_FOLDER}/config'

        self.config_dir = config_dir
        self.deploy_env_suffix = self.gcp_config['deploy_env_storage_suffix']
        self.job_config = read_yamlfile_env_suffix(f'{config_dir}/{config_filename}', self.deploy_env,
                                                   self.deploy_env_suffix)
        self.default_args = {
            "owner": "TBD",
            'capability': 'TBD',
            'severity': 'P3',
            'sub_capability': 'TBD',
            'business_impact': 'TBD',
            'customer_impact': 'TBD',
            "email": [],
            "email_on_failure": False,
            "email_on_retry": False,
            "retries": 3,
            "retry_delay": timedelta(minutes=1),
            "retry_exponential_backoff": True
        }

        self.local_tz = LOCAL_TZ

    def get_output_file_job(self, dag_config):
        return PythonOperator(
            task_id="get_output_file_name",
            python_callable=self.get_output_file_name,
            op_kwargs={
                'dag_config': dag_config}
        )

    def build_preprocessing_task_group(self, dag_id: str, dag_config: dict):
        with TaskGroup(group_id='preprocessing') as preprocessing:
            build_control_record_saving_job = PythonOperator(
                task_id='save_job_to_control_table',
                trigger_rule='all_success',
                python_callable=self.build_control_record_saving_task,
                op_kwargs={
                    'file_name': "{{  dag_run.conf['file_name'] }}",
                    'output_dir': f"gs://{dag_config.get(consts.DESTINATION_BUCKET)}",
                    'status': "STARTED",
                    'file_create_dt': "{{ dag_run.conf['file_create_dt'] }}",
                    'cycle_list': "{{ ti.xcom_pull(task_ids='extract_cycle_details', key='cycle_list') }}"
                }
            )
            build_control_record_saving_job

        return preprocessing

    def last_execution_check(self, dag_id: str, param_table: str, **context):
        client = bigquery.Client()
        execution_check_qry = f"""
                                 WITH
                                   max_start_time AS (
                                   SELECT
                                     MAX(load_timestamp) AS DT
                                   FROM
                                     {param_table}
                                   WHERE
                                     dag_id IN ('{dag_id}')
                                     AND JSON_EXTRACT(job_params, '$.status') = 'STARTED'),
                                   max_complete_time AS (SELECT
                                     MAX(load_timestamp) AS DT
                                   FROM
                                     {param_table}
                                   WHERE
                                     dag_id IN ('{dag_id}')
                                     AND JSON_EXTRACT(job_params, '$.status') = 'COMPLETED')
                                 SELECT
                                   IF(max_start_time.DT > max_complete_time.DT, FALSE, TRUE) AS result
                                 FROM
                                   max_start_time,max_complete_time
                              """
        execution_check_result = client.query(execution_check_qry).result().to_dataframe()
        execution_check_value = execution_check_result['result'].values[0]

        if execution_check_value:
            pass
        else:
            raise AirflowException(f"Earlier run for DAG : {dag_id} failed as per {param_table} table")

    def build_control_record_saving_task(self, file_name: str, output_dir: str, file_create_dt, status, cycle_list,
                                         **context):
        job_params_str = json.dumps({
            'source_filename': file_name,
            'extract_path': output_dir,
            'status': status,
            'file_create_dt': file_create_dt,
            'cycle_list': cycle_list
        })
        save_job_to_control_table(job_params_str, **context)

    def dependent_job_success_validator_task(self, dag_config: dict, file_create_dt, sa10_rec_count):
        logger.info(f"validating success fo dependent job for file_create_dt: {file_create_dt}")
        parallel_dag_id = dag_config.get("parallel_dag_id")
        logger.info(f"Parallel Dag id : {parallel_dag_id}")
        if int(sa10_rec_count) < 1:
            logger.info(f"No detail part in file. SA10_REC_COUNT is {sa10_rec_count}")
            return "End"
        if parallel_dag_id:
            logger.info(f"Checking if dag is completed {parallel_dag_id}")
            bigquery_client = bigquery.Client()
            table_details = dag_config.get('control_table')
            parallel_dag_id = "','".join(parallel_dag_id)
            parallel_dag_id = f"'{parallel_dag_id}'"
            job_check_query = f"""
                                 SELECT COUNT(1) AS cnt
                                 FROM {table_details}
                                 WHERE dag_id in ({parallel_dag_id})
                                 AND JSON_EXTRACT_SCALAR(job_params,'$.file_create_dt')="{file_create_dt}"
                             """
            logger.info(f'Query : {job_check_query}')
            result = bigquery_client.query(job_check_query).result().to_dataframe()
            job_check_count = result['cnt'].values[0]
            logger.info(f'Check count for above query : {job_check_count}')
            if int(job_check_count) == int(dag_config.get("dependent_job_count")):
                logger.info("All dependent jobs are completed.")
                return "get_output_file_name"
            else:
                logger.info("All dependent jobs are yet to complete.")
                return "End"
        else:
            logger.info("No dependent Jobs")
            return "get_output_file_name"

    def dependent_job_success_validator_job(self, dag_config: dict):
        return BranchPythonOperator(
            task_id='Validation_task',
            python_callable=self.dependent_job_success_validator_task,
            op_kwargs={
                'dag_config': dag_config,
                'file_create_dt': "{{ dag_run.conf['file_create_dt'] }}",
                'sa10_rec_count': "{{ dag_run.conf['sa10_rec_count'] }}"
            }
        )

    def build_postprocessing_task_group(self, dag_id: str, dag_config: dict):
        with TaskGroup(group_id='postprocessing') as postprocessing:
            build_control_record_saving_job = PythonOperator(
                task_id='save_job_to_control_table',
                trigger_rule='all_success',
                python_callable=self.build_control_record_saving_task,
                op_kwargs={
                    'file_name': "{{ dag_run.conf['file_name'] }}",
                    'output_dir': f"gs://{dag_config.get(consts.DESTINATION_BUCKET)}",
                    'file_create_dt': "{{ dag_run.conf['file_create_dt'] }}",
                    'status': "COMPLETED",
                    'cycle_list': "{{ ti.xcom_pull(task_ids='extract_cycle_details', key='cycle_list') }}"
                }
            )
            build_control_record_saving_job

        return postprocessing

    def get_output_file_name(self, dag_config: dict, **context):
        output_file_config = dag_config.get('output_file')
        output_file_extension = read_env_filepattern(output_file_config.get('filename_extension'), self.deploy_env)
        output_file_name = f"{output_file_config.get('output_filename')}_{JOB_DATE_TIME}.{output_file_extension}"

        context['task_instance'].xcom_push(key='output_file_name', value=output_file_name)
        logger.info(f"Output file name: {output_file_name}")

    def get_batch_num_val(self, dag_id):
        var_dict = Variable.get(dag_id, deserialize_json=True, default_var={})
        return var_dict.get('batch_num_val', {}) if var_dict else None

    def get_body_segment_list(self, dag_config: dict):
        bigquery_config = dag_config.get(consts.BIGQUERY)
        segment_configs = bigquery_config.get(consts.SEGMENT_ARGS)

        return [segment_name for segment_name in segment_configs]

    def append_to_landing_tables(self, dag_id: str, dag_config: dict, segment_name: str):
        client = bigquery.Client()
        logger.info(f"current output data will merged into output landing tables for : {dag_id}")
        bigquery_config = dag_config.get(consts.BIGQUERY)
        segment_configs = bigquery_config.get(consts.SEGMENT_ARGS)
        output_table = segment_configs.get(segment_name).get('output_table_name')
        output_table_name = f"pcb-{DEPLOY_ENV}-landing.domain_account_management.{output_table}"
        input_table = f"{output_table}_{consts.EXTERNAL_TABLE_SUFFIX}"
        input_table_name = f"pcb-{DEPLOY_ENV}-processing.domain_account_management.{input_table}"
        loading_sql = f"""
                        CREATE TABLE IF NOT EXISTS
                        {output_table_name}
                        AS
                        SELECT *
                        FROM {input_table_name}
                        LIMIT 0;

                        INSERT INTO {output_table_name}
                        SELECT *
                        FROM {input_table_name};
                    """
        loading_sql_results = client.query(loading_sql)
        loading_sql_results.result()
        if loading_sql_results.done() and loading_sql_results.error_result is None:
            logger.info(f"Successfully merged {output_table_name} with data from {input_table_name}")
        else:
            raise AirflowFailException(f"failed to create {output_table_name}")

    def append_landing_tables_task(self, dag_id: str, dag_config: dict, segment_name: str):
        return PythonOperator(
            task_id=f"append_to_landing_{segment_name}_table",
            python_callable=self.append_to_landing_tables,
            op_kwargs={'dag_config': dag_config,
                       'dag_id': dag_id,
                       'segment_name': segment_name}
        )

    def build_append_task_groups(self, dag_id, dag_config: dict, body_segments: Union[list, set]):
        task_groups = []

        for segment_name in body_segments:
            task_groups.append(self.append_landing_tables_task(dag_id, dag_config, segment_name))

        return task_groups

    def column_to_file(self, dag_config: dict, file_name: str, cycle_list, **context):
        client = bigquery.Client()
        storage_client = storage.Client()
        output_file_config = dag_config.get('output_file')
        column_name = output_file_config.get('column')
        table_detail = output_file_config.get('table_detail')
        if output_file_config.get('order_by'):
            order_by_clause = output_file_config.get('order_by')
        else:
            order_by_clause = ''
        cycle_list = [int(num.strip()) for num in cycle_list.strip('[]').split(',')]
        for cycle_no in cycle_list:
            output_folder = get_output_folder_path(dag_config).replace(CYCLE_NUM_PLACEHOLDER, str(cycle_no))
            file_name_cycle = file_name.replace(CYCLE_NUM_PLACEHOLDER, str(cycle_no).zfill(2))

            bucket_name = output_folder.split('/')[2]
            object_path = "/".join(output_folder.split('/')[3:])
            file_path = f"{object_path}/{file_name_cycle}" if object_path else file_name_cycle

            logger.info(f"Bucket Name: {bucket_name}")
            logger.info(f"File Path in GCS: {file_path}")

            bucket = storage_client.lookup_bucket(bucket_name)

            logger.info(f"Checking objects in bucket: {bucket_name} .")
            blobs = bucket.list_blobs(prefix=file_path)
            for blob in blobs:
                file_name = blob.name.rsplit('/', 1)[-1]
                file_name_check = output_file_config.get('output_filename').replace(CYCLE_NUM_PLACEHOLDER, str(cycle_no).zfill(2))
                logger.info(f"Checking file name: {file_name_check}")
                if file_name.startswith(file_name_check):
                    # Removing manul cleanup dependency while re-run
                    logger.info(f"Deleting existing blob: {blob.name}")
                    blob.delete()
            logger.info(f"Completed checking objects in bucket: {bucket_name} .")

            # table_order = ["HEDR", "DETL", "TRLR"]
            table_order = output_file_config.get('table_order')

            bucket = storage_client.lookup_bucket(bucket_name)
            blob = bucket.blob(file_path)

            record_written_count = 0

            with blob.open('wb') as file_stream:
                for segment in table_order:
                    table = table_detail.get(segment)
                    if table:
                        query = f"""
                            SELECT {column_name}
                            FROM {table}
                            WHERE cycle_no = {cycle_no}
                            {order_by_clause}
                            """
                        logger.info(f"Executing query:{query}")
                        results = client.query(query).result()
                        for row in results:
                            record = getattr(row, column_name)
                            value = record + '\n'
                            rec_in_bytes = value.encode('cp1252')
                            file_stream.write(rec_in_bytes)
                            if (record_written_count + 1) % 10000 == 0:
                                logger.info(f"Processed {record_written_count+1} records...")
                            record_written_count += 1
            logger.info(f"Column data from BigQuery dumped to {file_path}, number of records written: {record_written_count - 1}")

    def dump_column_task(self, dag_config: dict, **context):
        return PythonOperator(
            task_id='creating_output_file',
            python_callable=self.column_to_file,
            op_kwargs={'dag_config': dag_config,
                       'file_name': "{{ ti.xcom_pull(task_ids='get_output_file_name', key='output_file_name') }}",
                       'cycle_list': "{{ ti.xcom_pull(task_ids='extract_cycle_details', key='cycle_list') }}"
                       }
        )

    def load_output_tables(self, segment_configs: dict, segment_name: str, file_create_dt: str, file_name: str,
                           cycle_list):
        client = bigquery.Client()
        logger.info(f"file create date for this execution is: {file_create_dt}")
        output_table = segment_configs.get(segment_name).get('output_table_name')
        file_name = file_name.split('/')[1]
        logger.info(f"file name: {file_name}")
        output_table_name = f"{output_table}_{consts.EXTERNAL_TABLE_SUFFIX}"
        logger.info(f"output table name for this execution is: {output_table_name}")
        sql_file = f"{output_table}.sql"
        if sql_file is not None and len(sql_file) > 0:
            logger.info(f"Sql file for the task is: {sql_file}")
            create_table_sql = read_file_env(SQL_FILE_PATH + sql_file, self.deploy_env)
            extract_table = client.query(create_table_sql.replace(FILE_CREATE_DT_PLACEHOLDER, file_create_dt)
                                         .replace(OUTPUT_TABLE_PLACEHOLDER, output_table_name)
                                         .replace(FILE_NAME, file_name))
            extract_table.result()
            if extract_table.done() and extract_table.error_result is None:
                logger.info("Extract job completed successfully")
            else:
                raise AirflowFailException(f"Extract job not completed due to {extract_table.error_result}")

    def extract_output_data_task(self, dag_config: dict, bigquery_config: dict, segment_name):
        segment_configs = bigquery_config.get(consts.SEGMENT_ARGS)
        source_dag_id = dag_config.get('source_dag_id')
        task_id = f"load_output_table_{segment_name}"
        return PythonOperator(
            task_id=task_id,
            trigger_rule='all_success',
            python_callable=self.load_output_tables,
            op_kwargs={'bigquery_config': bigquery_config,
                       'segment_configs': segment_configs,
                       'segment_name': segment_name,
                       'source_dag_id': source_dag_id,
                       'file_create_dt': "{{ dag_run.conf['file_create_dt'] }}",
                       'file_name': "{{ dag_run.conf['file_name'] }}",
                       'cycle_list': "{{ ti.xcom_pull(task_ids='extract_cycle_details', key='cycle_list') }}"
                       }
        )

    def extract_cycle_details(self, dag_config: dict, file_create_dt, file_name, **context):
        cycle_config = dag_config.get('cycle_details')
        client = bigquery.Client()
        file_name = file_name.split('/')[1]
        for tables in cycle_config:
            cycle_job = tables.get('cycle_job')
            table = tables.get('table')
            domain = tables.get('domain')
            column = tables.get('column')
            logger.info(f"Fetching cycle details for {cycle_job} ")
            loading_query = f"""
                SELECT
                  ARRAY(SELECT DISTINCT({column})
                FROM
                  pcb-{DEPLOY_ENV}-{domain}.{table}
                WHERE
                  FILE_CREATE_DT = '{file_create_dt}'
                AND
                  FILE_NAME = '{file_name}'
                ORDER BY {column}) AS cycle_list
            """
            logger.info(f'SQL: {loading_query}')
            results = client.query(loading_query).result().to_dataframe()
            cycle_list = (results['cycle_list'].values[0])
            if isinstance(cycle_list, np.ndarray):
                cycle_list = cycle_list.tolist()
            cycle_list = [int(float(item)) for item in cycle_list]
            logger.info(f'Values for {cycle_job} - Cycle_list {cycle_list}')
            context['task_instance'].xcom_push(key='cycle_list', value=cycle_list)

    def extract_cycle_details_task(self, dag_config: dict, **context):
        return PythonOperator(
            task_id='extract_cycle_details',
            trigger_rule='all_success',
            python_callable=self.extract_cycle_details,
            op_kwargs={'dag_config': dag_config,
                       'file_create_dt': "{{ dag_run.conf['file_create_dt'] }}",
                       'file_name': "{{ dag_run.conf['file_name']}}"
                       }
        )

    def create_output_data(self, dag_config: dict, body_segments: Union[list, set]):
        task_groups = []

        for segment_name in body_segments:
            bigquery_config = dag_config.get(consts.BIGQUERY)
            task_groups.append(self.extract_output_data_task(dag_config, bigquery_config, segment_name))

        return task_groups

    def output_file_creation_job(self, dag_id: str, **context):
        return TriggerDagRunOperator(
            task_id=f"trigger_{self.job_config.get(dag_id).get('trigger_dag_id')}",
            trigger_dag_id=self.job_config.get(dag_id).get('trigger_dag_id'),
            conf={'file_create_dt': "{{ dag_run.conf['file_create_dt'] }}",
                  'cycle_list': "{{ ti.xcom_pull(task_ids='extract_cycle_details', key='cycle_list') }}",
                  'file_name': "{{ dag_run.conf['file_name']}}",
                  'sa10_rec_count': "{{ dag_run.conf['sa10_rec_count'] }}"
                  },
            wait_for_completion=False,
            trigger_rule='all_success',
            retries=0,
            poke_interval=60,
        )

    def outbound_decision(self, dag_id: str):
        var_dict = Variable.get(dag_id, deserialize_json=True, default_var={})
        outbound_trigger = var_dict.get('outbound_trigger', True)
        if outbound_trigger:
            return self.output_file_creation_job(dag_id)
        else:
            return DummyOperator(task_id="skipped_outbound_file_gen")

    def oracle_loader_job(self, dag_id: str, **context):
        return TriggerDagRunOperator(
            task_id=f"trigger_{self.job_config.get(dag_id).get('oracle_dag_id')}",
            trigger_dag_id=self.job_config.get(dag_id).get('oracle_dag_id'),
            conf={'file_create_dt': "{{ dag_run.conf['file_create_dt'] }}",
                  'file_name': "{{ dag_run.conf['file_name'] }}"
                  },
            wait_for_completion=False,
            trigger_rule='all_success',
            retries=0,
            poke_interval=60,
        )

    def oracle_decision(self, dag_id: str, dag_config: dict, **context):
        if dag_config.get('oracle_dag_id'):
            return self.oracle_loader_job(dag_id)
        else:
            return DummyOperator(task_id="skipped_oracle_loader_trigger")

    def create_dag(self, dag_id: str, dag_config: dict) -> DAG:
        """
        Main DAG which is triggered to extract output file
        """
        dag = DAG(
            dag_id=dag_id,
            default_args=self.default_args,
            start_date=datetime(2023, 1, 1, tzinfo=self.local_tz),
            dagrun_timeout=timedelta(hours=5),
            max_active_runs=1,
            catchup=False,
            schedule=None,
            is_paused_upon_creation=True
        )
        with (dag):
            if dag_config.get(consts.READ_PAUSE_DEPLOY_CONFIG):
                is_paused = read_pause_unpause_setting(OUTPUT_FILE_CREATION, self.deploy_env)
                pause_unpause_dag(dag, is_paused)

            dependent_job_validator = self.dependent_job_success_validator_job(dag_config)
            get_output_file_job = self.get_output_file_job(dag_config)
            control_table_task = self.build_postprocessing_task_group(dag_id, dag_config)
            pre_control_table_task = self.build_preprocessing_task_group(dag_id, dag_config)
            append_landing_tables_task = self.build_append_task_groups(dag_id, dag_config,
                                                                       self.get_body_segment_list(dag_config))
            dump_column = self.dump_column_task(dag_config)
            fetch_cycle_details = self.extract_cycle_details_task(dag_config)
            outbound_decision_job = self.outbound_decision(dag_id)
            oracle_decision_job = self.oracle_decision(dag_id, dag_config)
            start = DummyOperator(task_id='Start')
            end = DummyOperator(task_id='End', trigger_rule=TriggerRule.NONE_FAILED_MIN_ONE_SUCCESS)

            start >> dependent_job_validator
            dependent_job_validator >> end
            dependent_job_validator >> get_output_file_job >> fetch_cycle_details >> pre_control_table_task
            leading_task = pre_control_table_task

            segment_dependency_groups = dag_config.get(consts.BIGQUERY).get(consts.SEGMENT_DEPENDENCY_GROUPS)

            for segment_dependency_group in segment_dependency_groups:
                group_name = segment_dependency_group.get(consts.GROUP_NAME)
                group_start = DummyOperator(task_id=f'{group_name}_start')
                group_end = DummyOperator(task_id=f'{group_name}_end')
                leading_task >> group_start
                leading_task = group_end
                group_segments_str = segment_dependency_group.get(consts.SEGMENTS)
                group_segment_list = set([segment.strip() for segment in group_segments_str.split(consts.COMMA)])

                group_tasks = self.create_output_data(dag_config, group_segment_list)

                group_start >> group_tasks >> group_end

            (leading_task >> dump_column >> append_landing_tables_task >> control_table_task >> outbound_decision_job
             >> oracle_decision_job >> end)

        return add_tags(dag)

    def create_dags(self) -> dict:

        if self.job_config:
            dags = {}

            for job_id, config in self.job_config.items():
                self.default_args.update(config.get(consts.DEFAULT_ARGS))
                dags[job_id] = self.create_dag(job_id, config)
                dags[job_id].tags = config.get(consts.TAGS)

            return dags
        else:
            return {}
