INSERT INTO `pcb-{env}-landing.domain_scoring.SCORING_PREP_WK_ACE_FINAL`
(
  APP_NUMBER,
...
WITH
  ADM_APP_DATA AS (
    SELECT DISTINCT
      ...
      CAST(adm_std_app_data.APA_DATE_STATUS_YYYYMMDD AS integer)
        AS APA_DATE_STATUS_YYYYMMDD,
      CAST(adm_std_app_data.APA_TIME_STATUS_HHMMSS AS integer)
        AS APA_TIME_STATUS_HHMMSS,
      adm_std_app_data.APA_STATUS,
      adm_std_app_data.APA_QUEUE_ID,
      adm_std_app_data.APA_ASSIGNED_CREDIT_LIMIT,
      CAST(adm_std_app_data.APA_DATE_ENTERED_YYYYMMDD AS integer)
        AS APA_DATE_ENTERED_YYYYMMDD,
    ...
    FROM
      `pcb-{env}-curated.domain_customer_acquisition.ADM_STD_DISP_APP_DATA`
        adm_std_app_data
    LEFT JOIN
      `pcb-{env}-curated.domain_customer_acquisition.ADM_STD_DISP_TS2T_TMPL_INFO`
        adm_std_tmpl_info
      ON
  ...
  ADM_DE_DATA AS (
    SELECT DISTINCT
      ...
      REPLACE(TRIM(adm_disp_dt_elmt.ele_element_name), ' ', '_')
        AS ele_element_name_clean,
      CAST(adm_disp_dt_elmt.ELE_NUMERIC_VALUE AS FLOAT64) / 100000.0
        AS ELE_NUMERIC_VALUE2
    FROM
      `pcb-{env}-curated.domain_customer_acquisition.ADM_STD_DISP_APP_DATA`
        adm_disp_app_data
    LEFT JOIN
    ...
    WHERE
      ...
      AND adm_disp_dt_elmt.ele_element_name
        IN (
          'BUREAU_SCORE',
          'BUREAU_SELECTED',
          'SCORECARD_SELECTED',
  ...
  ADM_DE_NUMERIC AS (
    ...
    FROM
      (
        SELECT DISTINCT
          apa_app_num, EXECUTION_ID, ELE_NUMERIC_VALUE2, ele_element_name_clean
        FROM ADM_DE_DATA
        WHERE ELE_ELEMENT_TYPE = 'N'
      )
        PIVOT(
          ANY_VALUE(ELE_NUMERIC_VALUE2)
            FOR
              ele_element_name_clean IN (
                'BUREAU_SCORE',
                'APP_SCORE',
                'BNI_SCORE'))
  ),
  ADM_DE_ALPHA AS (
    ...
    FROM
      (
        SELECT DISTINCT
          apa_app_num, EXECUTION_ID, ELE_ALPHA_VALUE, ele_element_name_clean
        FROM ADM_DE_DATA
        WHERE ELE_ELEMENT_TYPE = 'A'
      )
        PIVOT(
          ANY_VALUE(ELE_ALPHA_VALUE)
            FOR
              ele_element_name_clean IN (
                'BUREAU_SELECTED',
                'SCORECARD_SELECTED',
  ...
  ADM_JOINED AS (
    ...
    WHERE
      adm_data.apa_status NOT IN (
        'VOID',
        'VOID APP')
      AND NOT EXISTS(
        SELECT 1
        FROM
          ADM_APP_DATA adm_void
        WHERE
          adm_void.apa_app_num = adm_data.apa_app_num
          AND adm_void.apa_status IN (
            'VOID',
            'VOID APP')
      )
  )
SELECT DISTINCT
  CAST(apa_app_num AS STRING) AS APP_NUMBER,
  CAST(apa_assigned_credit_limit AS STRING) AS CREDIT_LIMIT_GRANTED,
  CAST(PARSE_DATE('%Y%m%d', NULLIF(apa_date_final_disposition, '')) AS STRING)
    AS DATE_FINAL_DISPOSITION,
  CAST(
    PARSE_DATE('%Y%m%d', SAFE_CAST(apa_date_entered_yyyymmdd AS string))
    AS STRING) AS APP_ENTERED_DATE,
  CAST(
    PARSE_DATE('%Y%m%d', SAFE_CAST(apa_date_status_yyyymmdd AS string))
    AS STRING) AS APP_MODIFIED_DATE,
  CAST(apa_time_status_hhmmss AS STRING) AS APP_MODIFIED_TIME,
  CAST(
    CASE
      WHEN apa_status = 'NEWACCOUNT' THEN 'APPROVED'
      WHEN apa_status = 'DECLINE' AND apa_queue_id != 'AMLVERID' THEN 'DECLINED'
      WHEN apa_status IN ('WITHDRAW', 'TRUE DUPL', 'NORESPONSE') THEN 'DECLINED'
      ELSE 'PENDING'
      END
      AS STRING) AS APP_STATUS,
  CAST(APA_STRATEGY AS STRING) AS APA_STRATEGY,
  CAST(cb_type AS STRING) AS CB_TYPE,
  ...
  CAST(scorecard_id AS STRING) AS SCORECARD_ID,
  CAST(scorecard_selected AS STRING) AS SCORECARD_SELECTED,
  CAST(dtc_account_custom_data_40 AS STRING) AS P_FI_CE_EN_EMP_STAT,
  CAST(dtc_acquisition_strategy_code AS STRING)
    AS DTC_ACQUISITION_STRATEGY_CODE,
  CAST(dtc_promo_solicitation_code AS STRING) AS DTC_PROMO_SOLICITATION_CODE,
  CAST(DTC_STATE AS STRING) AS P_PI_CA_EN_PROV,
  SAFE_CAST(dtc_acquisition_strategy_code AS STRING) AS APP_SOURCE_CODE,
  CAST(SAFE_CAST(dtc_promo_solicitation_code AS integer) AS STRING)
    AS PROMO_CODE,
  CAST(-999 AS STRING) AS TOT_NUM_TRADES,
  CAST(-999 AS STRING) AS CLORTRADES,
  CAST(-999 AS STRING) AS MAJ_NUM_MDR,
  CAST(-999 AS STRING) AS TU_P_ESG_NUM_MTH_FLE,
  CAST(-999 AS STRING) AS TU_P_ESG_NUM_REV_TR,
  CAST(-999 AS STRING) AS CC_PO_B_PT_LNID,
  CAST(-999 AS STRING) AS P_FI_MTHLY_INCOME,
  CAST(app_score AS STRING) AS CC_PO_B_SC_RSK_S,
  CAST(
    CASE
      ...
      ELSE -999
      END
      AS STRING) AS CC_PO_B_SC_RSK_SCID,
  ...
  (
    SELECT DISTINCT
      *,
      ROW_NUMBER()
        OVER (
          PARTITION BY apa_app_num
          ORDER BY
            APA_DATE_STATUS_YYYYMMDD DESC,
            APA_TIME_STATUS_HHMMSS DESC,
            COALESCE(orgn_cb_score, app_score) DESC,
            COALESCE(
              DTC_ACCOUNT_CUSTOM_DATA_40, dtc_promo_solicitation_code) DESC,
            COALESCE(DTC_STATE) DESC
        ) AS rn
    FROM
      ADM_JOINED
  )
