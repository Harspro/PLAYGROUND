...
class BigQueryTableRemover:
    def __init__(self, config_dir: str = None, config_filename: str = None):
        ...
        if config_filename is None:
            config_filename = 'bigquery_table_remover_config.yaml'
    ...
    def drop_table(self, table_ref: str):
        bigquery.Client().delete_table(table_ref, not_found_ok=True)

    def drop_tables(self, table_configs):
        task_groups = []
        table_list = []
        ...
        for table_ref in table_list:
            ...
            with TaskGroup(group_id=table.table_id) as tgrp:
                ...
                drop_table = PythonOperator(
                    task_id=f"drop_{table.table_id}",
                    python_callable=self.drop_table,
                    op_kwargs={'table_ref': table_ref}
                )

                check_table >> back_up_original_table >> drop_table
                task_groups.append(tgrp)

        return task_groups

    def build_dag(self, dag_id: str, table_configs) -> DAG:
        logger.info(f'table_configs : {table_configs}')
        dag = DAG(
            dag_id=dag_id,
        ...
        with dag:
            ...
            tasks = self.drop_tables(table_configs)

            start_point >> tasks >> end_point
        return add_tags(dag)
    ...
    def create_dags(self) -> dict:
        ...
        if self.tables_config:
            for job_id, table_configs in self.tables_config.items():
                # print(f"==========={job_id}================")
                logger.info(f'table_configs are of type : {type(table_configs)}')
                dags[job_id] = self.build_dag(job_id, table_configs)

        return dags


globals().update(BigQueryTableRemover().create_dags())
