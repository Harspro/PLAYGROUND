EXPORT DATA
  OPTIONS ( uri = '{output_uri_processing}',
    format = 'CSV',
    header = TRUE,
    field_delimiter = '|',
    OVERWRITE = TRUE ) AS
WITH
  pcma_cdic_0501_latest_load AS (
  SELECT
    MAX(pcma_cdic_0501.REC_LOAD_TIMESTAMP) AS LATEST_REC_LOAD_TIMESTAMP
  FROM
    `pcb-{env}-curated.domain_cdic.PCMA_CDIC_0501` AS pcma_cdic_0501 )
SELECT
  pcma_cdic_0501.RELATIONSHIP_TYPE_CODE AS Relationship_Type_Code,
  REPLACE(pcma_cdic_0501.MI_RELATIONSHIP_TYPE, '|', '') AS MI_Relationship_Type,
  REPLACE(pcma_cdic_0501.DESCRIPTION, '|', '') AS Description
FROM
  `pcb-{env}-curated.domain_cdic.PCMA_CDIC_0501` AS pcma_cdic_0501
INNER JOIN
  pcma_cdic_0501_latest_load AS pcma_cdic_0501_ll
ON
  pcma_cdic_0501.REC_LOAD_TIMESTAMP = pcma_cdic_0501_ll.LATEST_REC_LOAD_TIMESTAMP;