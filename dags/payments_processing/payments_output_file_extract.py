...
class OutputFileCreation(ABC):
    ...
    def build_postprocessing_task_group(self, dag_id: str, dag_config: dict):
        with TaskGroup(group_id='postprocessing') as postprocessing:
            extract_outbound_job_stats = PythonOperator(
                task_id='extract_outbound_job_stats',
                trigger_rule='none_failed',
                python_callable=commons.extract_outbound_job_stats,
                op_kwargs={
                    'outbound_file_name': "{{ ti.xcom_pull(task_ids='get_output_file_name', key='output_file_name') }}",
                    'file_create_dt': "{{ dag_run.conf['file_create_dt'] }}",
                    'inbound_dag_run_id': "{{ dag_run.conf['inbound_dag_run_id'] }}",
                    'dag_config': dag_config}
            )
            ...
            extract_outbound_job_stats >> build_control_record_saving_job
    ...
    def generate_batch_num(self, dag_id: str, batch_num_table: str, **context):
        ...
        batch_param_table = f"pcb-{DEPLOY_ENV}-processing.domain_payments.{batch_num_table}"
    ...
    def append_to_landing_tables(self, dag_id: str, dag_config: dict, segment_name: str):
        ...
        output_table_name = f"pcb-{DEPLOY_ENV}-landing.domain_payments.{output_table}"
        input_table = f"{output_table}_{consts.EXTERNAL_TABLE_SUFFIX}_COLUMN_TF"
        input_table_name = f"pcb-{DEPLOY_ENV}-processing.domain_payments.{input_table}"
        loading_sql = f"""
                        CREATE TABLE IF NOT EXISTS
                        {output_table_name}
                        AS
    ...
    def increment_batch_num(self, dag_id: str, dag_config: dict, **context):
        bigquery_client = bigquery.Client()
        bigquery_config = dag_config.get(consts.BIGQUERY)
        batch_num_table = bigquery_config.get(BATCH_NUM_TABLE)
        batch_param_table = f"pcb-{DEPLOY_ENV}-processing.domain_payments.{batch_num_table}"
        current_batch_num_val = context['ti'].xcom_pull(task_ids='generate_batch_num', key='batch_num_value')
        logger.info(f"Current batch file num is: {current_batch_num_val}")
        new_batch_val = current_batch_num_val + 1
        insert_query = f"""INSERT INTO {batch_param_table}
                     (PARAM_NAME, PARAM_VALUE, CREATE_DT) VALUES
                     ( "BATCH_FILE_NUM", {new_batch_val}, CURRENT_DATE()) """
        logging.info(
            f"Inserting the incremented batch num value for {dag_id} into table {batch_param_table} "
            f"using the query {insert_query}")
    ...
    def get_initial_seq_value(self, segment_configs: dict, segment_name: str):
        client = bigquery.Client()
        initial_seq_value = 1
        seq_number_table = segment_configs.get(segment_name).get('sequence_number_table')
        if seq_number_table:
            logger.info(f"{seq_number_table} table found for {segment_name}, sequence will be calculated")
            seq_number_field = segment_configs.get(segment_name).get('sequence_number_field')
            initial_seq_value_qry = f""" SELECT IFNULL(MAX({seq_number_field}),0) AS SEQ_NUM
                                FROM pcb-{DEPLOY_ENV}-processing.domain_payments.{seq_number_table}"""
            initial_seq_value_result = client.query(initial_seq_value_qry).result().to_dataframe()
            initial_seq_value = initial_seq_value_result['SEQ_NUM'].values[0]
            if int(initial_seq_value) > 0:
                logger.info(f"initial seq value for {segment_name} is: {initial_seq_value}")
                return initial_seq_value
            else:
                prev_seq_number_field = segment_configs.get(segment_name).get('prev_sequence_number_field')
                prev_seq_number_table = segment_configs.get(segment_name).get('prev_sequence_number_table')
                if prev_seq_number_table:
                    initial_seq_value_qry = f""" SELECT IFNULL(MAX({prev_seq_number_field}),0) AS SEQ_NUM
                                                FROM pcb-{DEPLOY_ENV}-processing.domain_payments.{prev_seq_number_table}"""
                    initial_seq_value_result = client.query(initial_seq_value_qry).result().to_dataframe()
                    initial_seq_value = initial_seq_value_result['SEQ_NUM'].values[0]
                    if int(initial_seq_value) > 0:
                        logger.info(f"initial seq value for {segment_name} is: {initial_seq_value}")
                        ...
                        raise AirflowFailException(f"failed to find {initial_seq_value} for {segment_name}")
                else:
                    raise AirflowFailException(f"failed to find {prev_seq_number_table} for {segment_name}")
        else:
            logger.info(f"Default initial seq value for {segment_name} is: {initial_seq_value}")
            return initial_seq_value

    def increment_batch_num_task(self, dag_id: str, dag_config: dict):
        bigquery_config = dag_config.get(consts.BIGQUERY)
        batch_num_table = bigquery_config.get(BATCH_NUM_TABLE)
        if batch_num_table:
            return PythonOperator(
                task_id="increment_batch_num",
                python_callable=self.increment_batch_num,
                op_kwargs={'dag_config': dag_config,
                           'dag_id': dag_id}
            )
        else:
            return DummyOperator(task_id="skip_increment_batch_num")
    ...
    def load_output_tables(self, segment_configs: dict, segment_name: str, file_create_dt: str, batch_num_value: str):
        ...
        seq_initial_value = self.get_initial_seq_value(segment_configs, segment_name)
        output_table = segment_configs.get(segment_name).get('output_table_name')
        ...
        if sql_file is not None and len(sql_file) > 0:
            ...
            extract_table = client.query(create_table_sql.replace(INITIAL_SEQ_VALUE_PLACEHOLDER, str(seq_initial_value))
                                         .replace(FILE_CREATE_DT_PLACEHOLDER, file_create_dt)
                                         .replace(OUTPUT_TABLE_PLACEHOLDER, output_table_name)
                                         .replace(BATCH_NUM_PLACEHOLDER, batch_num_value))
    ...
    def move_output_file_to_landing(self, dag_config: dict):
        ...
        return GCSToGCSOperator(
            ...
            destination_object=f"{dag_config.get('destination_folder')}/{{{{ ti.xcom_pull(task_ids='get_output_file_name', key='output_file_name') }}}}"
        )
    ...
    def create_dag(self, dag_id: str, dag_config: dict) -> DAG:
        ...
        with (dag):
            ...
            fetch_batch_num = self.fetch_batch_num(dag_id, dag_config)
            ...
            increment_batch_num_task = self.increment_batch_num_task(dag_id, dag_config)
            move_output_file_to_landing = self.move_output_file_to_landing(dag_config)
            start = DummyOperator(task_id='Start')
            end = DummyOperator(task_id='End')

            start >> get_output_file_job >> fetch_batch_num >> cluster_creating_task
            ...
            (leading_task >> extractor_spark_job >> increment_batch_num_task >> append_landing_tables_task
