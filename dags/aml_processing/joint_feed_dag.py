...
FEED_CONFIG = commons.read_feed_config('feed.joint', 'aml_processing', 'aml_dag_config.yaml', )
DAG_NAME = "aml_joint_feed"
...
custom_default_args['business_impact'] = 'PCMC Authorized User information in Verafin will not be up-to-date if this process fails'
tags = commons.TAGS.copy()
joint_tag = custom_default_args['severity']
tags.append(joint_tag)

with DAG(
    DAG_NAME,
    default_args=custom_default_args,
    description="DAG to orchestrate the AML Joint Feed. Puts the output file in the outbound bucket",
    schedule='0 9 * * 1-5',
    catchup=False,
    ...
    run_joint_idl_to_bq = PythonOperator(
        task_id="run_joint_idl_to_bq",
    ...
    pipeline_start >> run_joint_idl_to_bq >> \
        export_table_to_gcs >> \
        compose_feed_file >> \
