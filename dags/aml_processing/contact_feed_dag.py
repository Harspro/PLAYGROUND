...
FEED_CONFIG = commons.read_feed_config('feed.contact', 'aml_processing', 'aml_dag_config.yaml', )
DAG_NAME = "aml_contact_feed"
...
severity_tag = custom_default_args['severity']
TAGS = ["team-defenders-alerts", "outbound", severity_tag]

custom_default_args['business_impact'] = 'Customer contact information in Verafin will not be up-to-date if this process fails'

with DAG(
    DAG_NAME,
    default_args=custom_default_args,
    description="DAG to orchestrate the AML Contact Feed. Puts the output file in the outbound bucket",
    schedule='0 9 * * 1-5',
    ...
    run_contact_idl_to_bq = PythonOperator(
        task_id="run_contact_idl_to_bq",
    ...
    pipeline_start >> run_contact_idl_to_bq >> \
        export_table_to_gcs >> \
        compose_feed_file >> \
