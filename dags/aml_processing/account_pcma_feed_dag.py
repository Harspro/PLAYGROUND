...
FEED_CONFIG = commons.read_feed_config('feed.account-pcma', 'aml_processing', 'aml_dag_config.yaml', )
DAG_NAME = "aml_account_pcma_feed"
...
custom_default_args['business_impact'] = 'PCMA account information in Verafin will not be up-to-date if this process fails'
tags = commons.TAGS.copy()
account_pcma_tag = custom_default_args['severity']
tags.append(account_pcma_tag)

with DAG(
    DAG_NAME,
    default_args=custom_default_args,
    description="DAG to orchestrate the AML Account pcma Feed. Puts the output file in the outbound bucket",
    schedule='0 9 * * 1-5',
    ...
    run_account_pcma_idl_to_bq = PythonOperator(
        task_id="run_account_pcma_idl_to_bq",
    ...
    pipeline_start >> run_account_pcma_idl_to_bq >> \
        export_table_to_gcs >> \
        compose_feed_file >> \
