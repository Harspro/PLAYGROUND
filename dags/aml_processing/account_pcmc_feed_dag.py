...
FEED_CONFIG = commons.read_feed_config('feed.account-pcmc', 'aml_processing', 'aml_dag_config.yaml', )
DAG_NAME = "aml_account_pcmc_feed"
...
custom_default_args['business_impact'] = 'PCMC account information in Verafin will not be up-to-date if this process fails'
tags = commons.TAGS.copy()
account_pcmc_tag = custom_default_args['severity']
tags.append(account_pcmc_tag)
with DAG(
    DAG_NAME,
    default_args=custom_default_args,
    description="DAG to orchestrate the AML Account PCMC Feed. Puts the output file in the outbound bucket",
    schedule='0 9 * * 1-5',
    ...
    run_account_pcmc_idl_to_bq = PythonOperator(
        task_id="run_account_pcmc_idl_to_bq",
        ...
        op_kwargs={"script": True},  # Use this only if you are using script in sql file instead of with statement
    ...
    pipeline_start >> run_account_pcmc_idl_to_bq >>\
        export_table_to_gcs >> \
        compose_feed_file >> \
